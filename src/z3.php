...
$zalecenia=array('0'=>'Nie masz koronawirusa. Więcej informacji znajdziesz na stronie <a href="https://www.pacjent.gov.pl">pacjent.gov.pl</a>',
...
'D'=>'Stan zagrożenia i wysokie ryzyko COVID - <b>konieczność wezwania karetki koronawirusowej</b>',
'R'=>'Stan zagrożenia i niskie ryzyko COVID <b>telefon do centrum ratownictwa (112)</b>');
...
function wytyczne($x) {
	...
	$stan=$post[$a.$q.$c.$d];
	global $stanx; $stanx=$stan;
	return $zalecenia[$stan];
}
...
$f=new TForm($b,$ef,$et);
if ($col!='white')  echo $f->naglowek('z4.php','Zalecenia i wytyczne (!)');
 else	echo $f->naglowek('index.php','Zalecenia i wytyczne');
//echo '<h1>Twoje zgłoszenie dot. stanu zdrowia zostało zarejestrowane.</h1>';
//echo '<h1>Twój numer zgłoszenia to: <strong><b>'.$idmd5.'</b></strong></h1>';
echo '<h2>'.$wytyczne.'</h2>';
echo '<p style="background-color:'.$col.';">'.$stanx.'</p>';
// jeśli powinien zarejestrować sie do bazy:
echo '<hr>';
if ($col!='white') {
  echo '<p style="color:red;">Pamiętaj, że to jest wersja testowa aplikacji!<br>Wysłane tym formularzem dane <b>nie trafią</b> do żadnych służb!<br>Wysłane dane trafią jedynie do bazy testowej!</p>';
  
  echo '<input type="hidden" id="cb1" name="cb1" value="'.v('cb1',$_REQUEST).'" />';
  echo '<input type="hidden" id="cb1a" name="cb1" value="'.v('cb1a',$_REQUEST).'" />';
  echo '<input type="hidden" id="cb2" name="cb2" value="'.v('cb2',$_REQUEST).'" />';
  echo '<input type="hidden" id="cb2a" name="cb2a" value="'.v('cb2a',$_REQUEST).'" />';
  echo '<input type="hidden" id="cb3" name="cb3" value="'.v('cb3',$_REQUEST).'" />';
  echo '<input type="hidden" id="cb3a" name="cb1" value="'.v('cb3a',$_REQUEST).'" />';
  echo '<input type="hidden" id="cb5" name="cb5" value="'.v('cb5',$_REQUEST).'" />';
  
  echo '<h3>Wypełnienie poniższego formularza zwiększa szansę na pomoc.</h3>';
  
  echo $f->polem('p', 'PESEL', 'i', 'pesel', '', 'maxlength="11"', null);
  echo $f->polem('t', 'Nr telefonu', 'i', 'Nr telefonu', '', 'maxlength="9"', null);
  echo $f->polem('Imie', 'Imię', 'i', 'imię', '', '', null);
  echo $f->polem('Nazwisko', 'Nazwisko', 'i', 'nazwisko', '', '', null);
  echo '<hr><h5>Obecne miejsce pobytu</h5>';
  echo $f->polem('Kod', 'Kod pocztowy', 'i', 'kod pocztowy', '', '', null);
  echo $f->polem('Miejscowosc', 'Miejscowość', 'i', 'miejscowość', '', '', null);
  echo $f->polem('Ulica', 'Ulica i nr domu', 'i', 'ulica', '', '', null);
  echo $f->polem('cb6', 'Wyrażam zgodę na przetwarzanie danych osobowych i udostępnianie lokalizacji', 'cb', '', '', 'onclick="gor(\'cb6\',\'d6\')"', null);
  echo $f->stopka('Wyślij zgłoszenie','z4.php');
} else
echo $f->stopka('Zamknij','index.php');
echo '</body></html>';
